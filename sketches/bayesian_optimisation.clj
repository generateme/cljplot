...
(show (draw-gp xs ys "GP regression with different kernel"
               "Kernel" kernels 
               (map vector kernels 
                    (map #(gp/gaussian-process xs ys {:kernel (k/kernel %)}) kernels))))

(let [xs [[0] [1] [-2] [-2.001]]
      y [1 -1 0.5 -0.6]]
  (println ((gp/gaussian-process xs y {:noise 0.00005}) 0))
  (show (draw-gp xs y "Gaussian processes with different noise"
                 "Lambda" [0.0005 0.1 2]
                 {:l1 (gp/gaussian-process xs y {:noise 0.00005})
                  :l2 (gp/gaussian-process xs y {:noise 0.1})
                  :l3 (gp/gaussian-process xs y {:noise 2})})))

(def gps {:l1 (gp/gaussian-process xs ys {:noise 0.00005})
        :l2 (gp/gaussian-process xs ys {:noise 0.1})
        :l3 (gp/gaussian-process xs ys {:noise 2})})
...
(def xs1 (repeatedly 50 #(vector (apply rnd/drand domain))))
(def ys1 (map toy-fn xs1))

(show (draw-gp xs1 ys1 "Gaussian processes with different noise"
               "Lambda"
               [0.5]
               {:l1 (gp/gaussian-process xs1 ys1 {:noise 10})
                :l2 (gp/gaussian-process xs1 ys1 {:noise 0.0001})} 300))
  ...
  ([gp cnt]
   (let [xs (range -3 3.1 0.1)
     ...
     (xy-chart {:width 700 :height 300}
               (-> (b/series [:grid])
                   (b/add-multi :line priors {} {:color (cycle (c/palette :category20c))}))
               (b/add-axes :bottom)
               (b/add-axes :left)
  ...
  (let [xxs (range -5 5.1 0.2)
    ...
    (xy-chart {:width 700 :height 300}
              (b/series [:grid]
                        ...
                        [:scatter (map vector xs2 ys2) {:size 8}])
              (b/add-axes :bottom)
              (b/add-axes :left)
              (b/add-label :top "Confidence intervals"))))

(show (draw-stddev (gp/gaussian-process xs2 ys2)))

(gp/posterior-samples (gp/gaussian-process xs2 ys2) xs2)
...
(def bo2 (opt/bayesian-optimization black-box-function2 {:bounds bounds2 :optimizer :gradient}))
...
(let [id 15
      pal (reverse (c/palette :rdylbu-9 ))
...
(def palette (reverse (c/palette :rdylbu-9 )))
